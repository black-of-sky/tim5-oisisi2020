...
public class RecipeContent extends JDialog {
	...
	public RecipeContent(int index) {
		...
		price = new JLabel(("Ukupna cena: " + RecipeController.getInstance().getTotalPrice(index)));
		down.add(price);
		...
		if (index == -1) {
			...
			addMedicineBtn.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					new AddMedicineToRecipe().setVisible(true);
					updateRecipePrice();
				}
			});
			...
			createRecipe.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (error.equals("")) {
						RecipeController.getInstance().create(jmbg);
						setVisible(false);
						dispose();
					} else {
	...
	public void updateRecipePrice() {
		price.setText("Ukupna cena:" + RecipeController.getInstance().getTotalPrice(index));
	}
}
